...
def generate_single_year_report(data, feature, year, granularity='Yearly', month=None):
    ...
    try:
        ...
        loan_status_table = filtered_data.groupby([feature, 'LOAN_STATUS']).size().unstack(fill_value=0)
        loan_status_table = loan_status_table.reindex(columns=['NPL', 'PAS', 'SET', 'SME'])

        # Add feature name to the first cell
        loan_status_table.columns.name = ''
        loan_status_table.index.name = feature
        ...
        if granularity in ['Yearly', 'Monthly']:
            ...
            ax.set_title(f'Distribution of {feature} with Loan_Status for the {granularity} {year}')
        elif granularity == 'Weekly':
